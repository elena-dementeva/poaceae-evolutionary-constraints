...
def filter_mirna_targets(
    ...
    if target_desc_keyword:
        df_filtered = df_filtered[
            df_filtered["Target_Desc."].str.contains(
                target_desc_keyword, case=False, na=False
            )
        ]
    ...
    print(
        f"Результаты сохранены в {output_file}. Отфильтровано строк: {len(df_filtered)}"
    )


def main():
    parser = argparse.ArgumentParser(
        description="Фильтрация результатов предсказания мишеней miRNA."
    )
    parser.add_argument(
        "-i", "--input", required=True, help="Входной файл (формат psRNATarget)"
    )
